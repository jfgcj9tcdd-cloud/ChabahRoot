...
load_detection_rules() {
    if [[ ! -f "$DETECTION_RULES_FILE" ]]; then
        log_alert "RULES" "Fichier de regles introuvable: $DETECTION_RULES_FILE"
        return 1
    fi

    if ! jq empty "$DETECTION_RULES_FILE" >/dev/null 2>&1; then
        log_error "RULES" "Fichier JSON invalide: $DETECTION_RULES_FILE"
        return 1
    fi

    log_info "RULES" "Regles chargees depuis $DETECTION_RULES_FILE"
}
    ...
    while IFS= read -r action; do
        ...
        case "$action" in
            journal)
                ...
                ;;
            email)
                log_info "ACTIONS" "Action email non implementee"
                ;;
            slack)
                log_info "ACTIONS" "Action slack non implementee"
                ;;
            *)
                log_warn "ACTIONS" "Action inconnue: $action"
                ;;
        esac
    done <<< "$actions"
}
...
main() {
    ...
    log_info "DETECTION" "Moteur M2 demarre"

    while IFS= read -r event_json; do
        [[ -z "${event_json// }" ]] && continue

        if ! jq empty >/dev/null 2>&1 <<< "$event_json"; then
            log_warn "DETECTION" "Evenement ignore car JSON invalide"
            continue
        fi
